...
| `tools/check_citations.py` | A quotation attributed to a requirement whose own body does not contain those words; this verifies even short normative quotations accepted by the restatement check, preserves explicit speakers through intervening prose, and checks every owner in an attached parenthetical (`F2`) |
