...
| `conformance/` | The suite: `run`, the fakes, the fixtures, `scenarios.tsv`; and `test-panel-trace`, the regression checks for the suite's Panel-membership comparator, for the fake past its script's last entry, and for `restrict_path`'s skip line, its refusals and its normalisation of a relative `PATH` entry |
