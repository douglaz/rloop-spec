...
but only F2's blocking forms fail the gate (ADR-0007).
...
tools/test_citation_gates.py for isolated controls. Exit 0 = no blocking findings
(advisories may be present), 1 = a blocking false quote.
"""
...
def find(docs, reqs):
    ...
    for document, text in docs.items():
        for unit in units(prose(text)):
            for owner, quote, tier in attributions(unit):
                ...
                if not phrase or not re.search(r"(?<!\w)" + re.escape(phrase) + r"(?!\w)", body):
                    bad.append((document, unit.start + quote.start, owner, quote.text, tier))
    return bad
...
def main():
    docs, reqs = load(ROOT)
    bad = find(docs, reqs)
    for document, position, owner, quote, tier in bad:
        source = reqs[owner].document if owner in reqs else "undefined owner"
        print(f"{tier} CITATION: {document}:{line_at(docs[document], position)} attributes "
              f"to {owner} ({source}) words absent from its body:")
        print(f"  {norm(quote)}")
    blocking = sum(tier == "BLOCKING" for *_, tier in bad)
    print(f"Quoted attributions: {blocking} blocking, {len(bad) - blocking} advisory." if bad else
