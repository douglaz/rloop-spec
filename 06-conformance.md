...
error the item asserts presence only: `RUN-22` sets what the message carries — `a message naming
the Seat — both, when both are — its model, and the reset the probe reported` — and no bound on it,
and `RUN-10` leaves the rest of the stream unspecified — `Progress and diagnostics go to standard
error and their wording is not specified`. The pick is scripted to fail. *Again the spawn record,
not the status, is what an Implementation that refuses only for the Manager's Seat fails. The
second Run is there because one row cannot tell a Seat read from its own model from a rule that
hardcodes `fable`, and a per-Seat or per-site lookup can get one Seat's row right and another's
wrong. A message naming both Seats unavailable when only one is passes here, since no sentence in
the set forbids it; the per-Seat reading stays asserted where it is observable, in
`probe-pick.md`'s `manager:unknown` and `implementer:unavailable`.* (`RUN-22`, `RUN-21`, `DIR-4`,
`RUN-10`)
