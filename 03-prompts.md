...
4. Settle what the brief depends on, before writing it:
   ...
   - If the task needs an implementation decision the specifications leave to the implementer — a choice of how, not of what — do not make it alone. Follow `RUN-20`: put the question and the options first to fable and astra, skipping any model that did not answer earlier in this Run and using its eligible counterpart instead. Their commands, run in this repository, are exactly:
     ...
     Before calling any adviser, choose each call's maximum duration, a total Consultation budget and time reserved to write {{FINISHED_FILE}}. Use your shell tool to bound every call through completion or termination; a tool returning while the adviser still runs is not completion. Keep the whole Consultation within that budget and leave enough of your turn to write {{FINISHED_FILE}}. Record the chosen durations, bounds used and each call's elapsed time, every model called and what it did — answered, did not answer with the reason, or refused — with the question, the answers and your choice in the brief, or in the report if blocked before writing a brief. If fewer than two models answered, stop and write {{FINISHED_FILE}} with `STATUS: blocked` and say the advisers did not answer sufficiently to form a quorum; do not say they could not agree or call it a clarification of the specifications. If at least two answered but the eligible advisers do not settle their disagreement, stop and write {{FINISHED_FILE}} with `STATUS: blocked`, saying the advisers could not agree, with the question, every answer and what you recommend. A choice the brief does not need to settle is the Implementer's to make.
...
Two kinds of question are not yours to answer alone. If this Round surfaced a point the repository's specifications or instructions leave ambiguous or contradictory — two readings that lead to different behaviour — do not choose a reading: choose A below with `STATUS: blocked`, and put the document, the passage, the readings and the clarification you recommend in the report. If it surfaced an implementation decision the specifications leave to the implementer — a choice of how, not of what — that the next brief must settle, follow `RUN-20` before writing it: put the question and the options first to fable and astra, using an eligible counterpart for any model that did not answer earlier in this Run. Run their commands in this repository exactly as follows:
claude -p "<your question>" --model claude-fable-5-1 --effort high --dangerously-skip-permissions --disallowedTools "Edit,Write,NotebookEdit,Bash(git checkout:*),Bash(git stash:*),Bash(git reset:*),Bash(git commit:*),Bash(git clean:*)"
codex exec -s read-only -m gpt-6-astra -c model_reasoning_effort=high "<your question>"
Do not settle the choice with only one answer: two distinct models must answer. Count a call only when it finishes within its bound, exits zero and states a position. Timeouts, non-zero exits, empty output, output consisting only of an error, refusals and prose with no position are not answers; whether hedged prose states a position is your judgment. For a non-answer use that model's counterpart — fable↔opus, astra↔sol — rather than roster order, and never call that non-answering model again in this Run. Try eligible models until two have answered or none remain within the budget. Seek one claude answer and one codex answer; when neither model of one vendor answers, two from the other vendor are enough, and you must identify the Consultation as single-vendor in both the brief and the report. Choose with two answering advisers who agree; if they disagree, ask the remaining eligible advisers the same question. The other pair uses exactly these commands:
claude -p "<your question>" --model claude-opus-5 --effort xhigh --dangerously-skip-permissions --disallowedTools "Edit,Write,NotebookEdit,Bash(git checkout:*),Bash(git stash:*),Bash(git reset:*),Bash(git commit:*),Bash(git clean:*)"
codex exec -s read-only -m gpt-5.6-sol -c model_reasoning_effort=xhigh "<your question>"
Before any adviser call, set a maximum duration for each call and a budget for the entire Consultation, reserving enough of your turn to write {{FINISHED_FILE}}. Enforce each bound through your shell tool until the call completes or is terminated, and stay within the total budget; a shell tool that returns while an adviser is still running has not completed that adviser call. In the next brief, or in the report when blocked before writing it, record the chosen durations, the bounds used and elapsed time for each call, every model you called and whether it answered, did not answer (and why), or refused, together with the question, all answers and your choice. With fewer than two answering models, choose A below with `STATUS: blocked` and report that the advisers did not answer sufficiently to form a quorum, not that they could not agree; this is not a clarification of the specifications. With at least two answers but disagreement the eligible advisers do not settle, choose A below with `STATUS: blocked` and say the advisers could not agree, giving the question, every answer and your recommendation.
